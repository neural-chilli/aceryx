...
func (s *Service) Restore(ctx context.Context, opts RestoreOptions) (RestoreResult, error) {
	...
	if err := s.pgRestoreDump(ctx, targetDB, extracted.dumpPath); err != nil {
		return RestoreResult{}, err
	}
...
func (s *Service) Verify(ctx context.Context, inputPath string) (VerifyResult, error) {
	...
	if err := s.pgRestoreList(ctx, extracted.dumpPath); err != nil {
		return VerifyResult{Status: "CORRUPT"}, err
	}
...
func (s *Service) pgRestoreDump(ctx context.Context, dbURL, dumpPath string) error {
	details, err := parseConn(dbURL)
	if err != nil {
		return err
	}
...
func (s *Service) pgRestoreList(ctx context.Context, dumpPath string) error {
